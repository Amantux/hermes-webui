...
def test_joplin_search_notes_returns_safe_snippets(monkeypatch):
    ...
    def fake_get(path, params=None, cfg_data=None):
        assert path == "/search"
...
def test_joplin_api_get_uses_authorization_header(monkeypatch):
    ...
    monkeypatch.setattr(routes, "_joplin_connection_from_config", lambda cfg_data=None: ("http://127.0.0.1:41184", "secret-token"))
...
def test_llm_wiki_search_notes_uses_workspace_sources(tmp_path, monkeypatch):
    from api import routes

    monkeypatch.delenv("WIKI_PATH", raising=False)
    monkeypatch.setattr(routes, "_llm_wiki_env_file_path", lambda _home: None)

    docs = tmp_path / "docs"
    docs.mkdir(parents=True, exist_ok=True)
    (docs / "intro.md").write_text("# Intro\nHermes wiki reference content.", encoding="utf-8")

    results, path_source = routes._llm_wiki_search_notes("Hermes", limit=10, workspace_path=tmp_path)

    assert path_source == "workspace"
    assert len(results) == 1
    assert results[0]["id"] == "intro.md"
    assert results[0]["source"] == "llm-wiki"


def test_llm_wiki_get_note_reads_workspace_note(tmp_path, monkeypatch):
    from api import routes
    ...
    (wiki / "agent.md").write_text("# Agent\nCompatibility notes.", encoding="utf-8")

    note, path_source = routes._llm_wiki_get_note("agent.md", workspace_path=tmp_path)

    assert path_source in {"workspace", "workspace_default"}
    assert note["source"] == "llm-wiki"
    assert note["title"] == "Agent"
    assert "Compatibility notes." in note["body"]


def test_llm_wiki_graph_extracts_markdown_links(tmp_path, monkeypatch):
    from api import routes
    ...
    graph, _source = routes._llm_wiki_graph(tmp_path, max_nodes=10, max_edges=10)

    node_ids = {node["id"] for node in graph["nodes"]}
    assert "alpha.md" in node_ids
    assert "beta.md" in node_ids
    assert {"source": "alpha.md", "target": "beta.md"} in graph["edges"]


def test_llm_wiki_path_global_default_uses_resolved_path(monkeypatch, tmp_path):
    from api import routes

    monkeypatch.setattr(routes, "_llm_wiki_resolve_path", lambda: (tmp_path, "available", True))

    path, source = routes._llm_wiki_path_for_workspace(None, {})

    assert path == tmp_path
    assert source == "global_default"


def test_llm_wiki_project_path_must_stay_inside_workspace(tmp_path):
    from api import routes

    project_cfg = tmp_path / ".hermes" / "project.yaml"
    project_cfg.parent.mkdir(parents=True, exist_ok=True)
    project_cfg.write_text("wiki:\n  path: ../../etc\n", encoding="utf-8")

    with pytest.raises(ValueError, match="inside the selected workspace"):
        routes._llm_wiki_path_for_workspace(tmp_path, {})


def test_notes_sources_route_supports_workspace_scope(monkeypatch, tmp_path):
    from urllib.parse import quote
    from api import routes

    monkeypatch.setenv("HERMES_WEBUI_EXTERNAL_NOTES_SOURCES", "1")
    monkeypatch.setattr(routes, "get_config", lambda: {"mcp_servers": {"profile-only": {"command": "x"}}})
    monkeypatch.setattr(routes, "_mcp_runtime_status_by_name", lambda: {})
    monkeypatch.setattr(routes, "_load_project_config_for_workspace", lambda _ws: {"mcp_servers": {"project-only": {"command": "y"}, "profile-only": None}})
    monkeypatch.setattr(routes, "resolve_trusted_workspace", lambda _raw: tmp_path)
    monkeypatch.setattr(routes, "_llm_wiki_env_file_path", lambda _home: None)

    handler = MagicMock()
    handler.path = f"/api/notes/sources?workspace={quote(str(tmp_path), safe='')}"
    routes._handle_notes_sources_list(handler)
    payload = _json_payload(handler)
    assert payload["config_scope"] == "project+profile"
    assert payload["workspace"] == str(tmp_path)


def test_notes_graph_route_returns_llm_wiki_graph(monkeypatch, tmp_path):
    ...
    docs = tmp_path / "docs"
    docs.mkdir(parents=True, exist_ok=True)
    (docs / "intro.md").write_text("# Intro\nWiki graph route.", encoding="utf-8")

    handler = SimpleNamespace(path=f"/api/notes/graph?workspace={quote(str(tmp_path), safe='')}")
    captured = {}
    ...
    with patch("api.routes.resolve_trusted_workspace", return_value=tmp_path), patch("api.routes.j", side_effect=fake_j):
        handled = routes.handle_get(handler, urlparse(handler.path))

    assert handled is None
    assert captured["status"] == 200
    assert captured["payload"]["source"] == "llm-wiki"
    assert captured["payload"]["nodes"]


def test_notes_search_route_returns_llm_wiki_results(monkeypatch, tmp_path):
    from types import SimpleNamespace
    from urllib.parse import quote, urlparse
    from unittest.mock import patch
    import api.routes as routes
    ...
    docs = tmp_path / "docs"
    docs.mkdir(parents=True, exist_ok=True)
    (docs / "intro.md").write_text("# Intro\nHermes route search content.", encoding="utf-8")

    handler = SimpleNamespace(path=f"/api/notes/search?source=llm-wiki&q=Hermes&workspace={quote(str(tmp_path), safe='')}")
    captured = {}
...
def test_notes_item_route_returns_llm_wiki_note(monkeypatch, tmp_path):
    from types import SimpleNamespace
    from urllib.parse import quote, urlparse
    from unittest.mock import patch
    import api.routes as routes
    ...
    docs = tmp_path / "docs"
    docs.mkdir(parents=True, exist_ok=True)
    (docs / "intro.md").write_text("# Intro\nHermes route note content.", encoding="utf-8")

    handler = SimpleNamespace(path=f"/api/notes/item?source=llm-wiki&id=intro.md&workspace={quote(str(tmp_path), safe='')}")
    captured = {}
    ...
    assert captured["status"] == 200
    assert captured["payload"]["source"] == "llm-wiki"
    assert captured["payload"]["note"]["id"] == "intro.md"


def test_notes_item_route_returns_404_for_missing_llm_wiki_note(monkeypatch, tmp_path):
    from types import SimpleNamespace
    from urllib.parse import quote, urlparse
    from unittest.mock import patch
    import api.routes as routes
    ...
    handler = SimpleNamespace(path=f"/api/notes/item?source=llm-wiki&id=missing.md&workspace={quote(str(tmp_path), safe='')}")
    captured = {}
